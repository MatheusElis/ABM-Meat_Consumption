...
amount_meat = function(meal, agent)
    if society:get(agent.id).MeatHab == 1 then
        if society:get(agent.id).Rsex == 1 then
            if meal == meal[1] then
        ...
        elseif society:get(agent.id).Rsex == 2 then
            if meal == meal[1] then
    ...
    elseif society:get(agent.id).MeatHab == 0 then
        std_gMeat = 0
        tax_gMeat_b = 0
        return std_gMeat, tax_gMeat_b
    end
...
tax_meat = function(meal, agent)
    if meal == meal[1] then
        tax_gMeat_b = ped_effect(week) * society:get(agent.id).std_gMeat
        return tax_gMeat_b
    elseif meal == meal[2] then
        tax_gMeat_l = ped_effect(week) * society:get(agent.id).std_gMeat
        return tax_gMeat_d
    elseif meal == meal[3] then
        tax_gMeat_d = ped_effect(week) * society:get(agent.id).std_gMeat
        return tax_gMeat_l
    end
end
...
--[[day_meat = function(meal, id)
    if meal == meal[3] then
        return gMeat.day = society:get(agent.id).tax_gMeat_b + society:get(agent.id).tax_gMeat_d + society:get(agent.id).tax_gMeat_l
    end
end--]]